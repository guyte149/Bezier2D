...
class AnimationConnection(object):

    def __init__(self, best_curve, ang1, p2, ang2):
        ...
        for i in xrange(0, 4):
            plot_point = self.ax.plot([], [], 'ro')[0]
            self.plot_list.append(plot_point)
        self.plot_list.append(self.ax.plot([], [])[0])
    ...
    def animate(self, i):
        ...
        t_list = np.linspace(0.0, 1.0, res)
        ...
        # setting data for control points
        self.plot_list[len(self.curves)].set_data(best_curve.c0[0], best_curve.c0[1])
        self.plot_list[len(self.curves) + 1].set_data(best_curve.c1[0], best_curve.c1[1])
        self.plot_list[len(self.curves) + 2].set_data(best_curve.c2[0], best_curve.c2[1])
        self.plot_list[len(self.curves) + 3].set_data(best_curve.c3[0], best_curve.c3[1])

        x_list = []
        y_list = []
        for i in xrange(0, int(res + 1), 1):
            x_list.append(self.best_curve(i / res)[0])
            y_list.append(self.best_curve(i / res)[1])

        self.plot_list[len(self.curves) + 4].set_data(x_list, y_list)
